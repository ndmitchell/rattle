...
main :: IO ()
main = let t1 = createState [(Cmd "cmd1" ((T 0),Required) (T 5) [] [] (Set.singleton "a") (Set.singleton "b") []) -- nonRecoverable ReadWrite hazard
                            ,(Cmd "cmd2" ((T 1),Required) (T 1) [] [] (Set.singleton "b") (Set.singleton "a") [])]

           t2 = createState [(Cmd "cmd1" ((T 0),Required) (T 5) [] [] (Set.singleton "a") (Set.singleton "b") []) -- No hazard (Seq cmd1 cmd2)
                            ,(Cmd "cmd2" ((T 1),Required) (T 1) [] [] (Set.singleton "b") (Set.singleton "c") [])]
         ...
         do
           ...
           case done t1r1 of
             (Hazard (ReadWriteHazard _ _ _ NonRecoverable) _) ->
               putStrLn $ "t1: " ++ (show $ (done t1r1 == done t1r2) && (done t1r2 == done t1r3))
             x ->
           ...
           t2r3 <- consSched t2
           case done t2r1 of
             (Tree _ _) -> putStrLn $ "t2: " ++ (show $ (done t2r1 == done t2r2) && (done t2r2 == done t2r3))
             x -> putStrLn $ "t2: failed: " ++ show x
           (Identity t3r1) <- return $ seqSched t3
           t3r2 <- originalSched t3
           t3r3 <- consSched t3
           case done t3r1 of
             (Hazard (WriteWriteHazard _ _ _) _) ->
               putStrLn $ "t3: " ++ (show $ (done t3r1 == done t3r2) && (done t3r2 == done t3r3))
