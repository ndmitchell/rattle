...
main = unless isMac $ withOutput $ \root -> do
    ...
    cs <- liftIO $ getDirectoryFiles "../test" ["C/*.c"]
    cs2 <- liftIO $ getDirectoryFiles "../test" ["C2/*.c"]
    let toO x = takeBaseName x <.> "o"
    let build = do
          withCmdOptions [Cwd "../test"] $ do
            cmd "pwd"
            forM_ cs $ \c -> cmd "gcc -o" [toO c, "-c", c]
            cmd "gcc -o" ["../output/Main" <.> exe] (map toO cs)
          cmd ["./Main" <.> exe]

        build2 = do -- pretend we moved things; so we updated our build script
          withCmdOptions [Cwd "../test"] $ do
            cmd "pwd"
            forM_ cs2 $ \c -> cmd "gcc -o" [toO c, "-c", c]
            cmd "gcc -o" ["../output/Main" <.> exe] (map toO cs)
          cmd ["./Main" <.> exe]
